...
\begin{itemize}
\item \textbf{Null hypothesis ($H_0$):} the "boring" default - there is no effect, change, difference, or anything interesting going on. Eg: the new checkout button color has no effect on conversion rate."
\item \textbf{Alternative hypothesis ($H_1$ or $H_a$):} what you are actually trying to find evidence for - there is an effect, change, difference, or something interesting. Eg: the new checkout button color changes conversion rate.
\end{itemize}
